...
parser.add_argument("--scanvi_n_epochs", default = False)
...
parser.add_argument("--capture_type_col", default = 'capture_type')
parser.add_argument("--biological_sample_col", default = 'study_accession')
parser.add_argument("--continuous_covariate_keys", default = 'pct_counts_mt,pct_counts_ribo,pct_counts_protocadherin')
...
print("scVI continuous_covariate_keys are: " + args.continuous_covariate_keys + "\n\n")
...
# two columns
# first is capture type (nucleus or cell)
# second is study/biological sample/etc
# if capture type col not in the obs slot, then 
# assume that this is just a value to be filled in
if args.capture_type_col not in adata.obs.columns:
    adata.obs[args.capture_type_col] = args.capture_type_col

adata.obs['capture_covariate'] = [a + '_' + b for a,b in zip(list(adata.obs[args.capture_type_col]),list(adata.obs[args.biological_sample_col]))]
if 'MajorCellType' in adata.obs.columns:
...
if args.continuous_covariate_keys != '':
...
cck = args.continuous_covariate_keys
if cck == '':
    print("running without continuous covariates")
    scvi.model.SCVI.setup_anndata(adata_ref,  layer="counts",
                              batch_key=covariate)
else:
    print("running with continuous covariates: " + cck)
    cck = cck.split(',')
    scvi.model.SCVI.setup_anndata(adata_ref,  layer="counts",
                                continuous_covariate_keys = cck,
                              batch_key=covariate)
...
if args.scanvi_model_out:
    ...
    #
    if args.scanvi_n_epochs:
        scanvi_epochs = int(args.scanvi_n_epochs)
    else:
    ...
    #
    adata_full.obsm[SCANVI_PREDICTION_KEY+"_soft"] = scanvi_query.predict(adata_full, soft = True)
    ...
    adata_full.obs[SCANVI_PREDICTION_KEY+"_max_score"] = adata_full.obsm[SCANVI_PREDICTION_KEY+"_soft"].max(axis=1) 
...
adata_full.obs['barcodei'] = adata_full.obs.index.values
