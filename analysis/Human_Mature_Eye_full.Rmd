...
<!--     - built against slightly inaccurate scANVI calls -->
<!--   2. Run against Stage 4 moels -->
<!--     - but there are two models (neural, nonneural) -->
<!--   3. Build a new model -->
<!--     - bleh -->
<!--     - maybe the best idea? -->
<!--     - twice? one for the CT and another for hrCT (high resolution) -->
<!--     - `obs$nobs` and `obs$nlabels` also have the hand-QC fail cells -->
...
## rebuild scVI models
step1: update the ref and query barcodes to REMOVE the ones that have "suspect" labels
```{r}
...
1. Use models made in Stage 6

<!-- ```{r, fig.width=12, fig.height=9} -->
<!-- obs_s6 <- pull_obs('~/data/scEiaD_modeling/hs111_mature_eye_20241121_stage6_2000hvg_200e_30l.hrCT.obs.csv.gz', machine_label = 'hrCT_scANVI_step6') -->
<!-- obs_s6$labels %>% filter(aMCT != mMCT) %>% data.frame -->


<!-- obs_s6$obs %>%  -->
<!--   filter(suspect != 'Yes') %>%  -->
<!--   left_join(obs_s6$labels, by = 'leiden3') %>%  -->
<!--   ggplot(aes(x=umap1,y=umap2)) + -->
<!--   scattermore::geom_scattermore(aes(color = hrCT_scANVI_step6), pointsize = 0.8, alpha = 0.5) + -->
<!--   ggrepel::geom_label_repel(data = . %>% group_by(hrCT_scANVI_step6) %>%  -->
<!--                               summarise(umap1 = median(umap1), -->
<!--                                         umap2 = median(umap2)), -->
<!--                             aes(label = hrCT_scANVI_step6, color = hrCT_scANVI_step6)) + -->
<!--   scale_color_manual(values = c(pals::alphabet2(), pals::glasbey()) %>% unname()) +  -->
<!--   cowplot::theme_cowplot() + theme(legend.position = "none") -->
