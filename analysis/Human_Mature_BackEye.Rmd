...
hs111_study <- hs111_backeye %>% 
  group_by(study_accession) %>% summarise(Count = n()) %>% filter(Count>1000)
set.seed(101294)
hs111_backeye$MajorCellType %>% table()
hs111_ref_bcs <- hs111_backeye %>% 
...
hs111_query_bcs <- hs111_backeye %>% 
  filter(!barcode %in% hs111_ref_bcs) %>% 
  pull(barcode)

hs111_ref_bcs %>% write(gzfile('~/git/scEiaD_modeling/data/hs111_mature_backeye_ref_bcs.20240830-02.csv.gz'))
hs111_query_bcs %>% write(gzfile('~/git/scEiaD_modeling/data/hs111_mature_backeye_query_bcs.20240830-02.csv.gz'))

```

## run scVI

now go to biowulf2:/data/OGVFB_BG/scEiaD/2024_02_28/snakeout/hs111_mature_backeye

```{bash, exec = FALSE}
# will take ~2-4 hours (depending on hpc usage)
mamba deactivate; mamba activate; bash ~/git/scEiaD_modeling/Snakemake.wrapper.sh ~/git/scEiaD_modeling/workflow/Snakefile ~/git/scEiaD_modeling/config/config_hs111_mature_backeye.yaml ~/git/scEiaD_modeling/config/cluster.json
```

## rsync output from biowulf2 to local computer
```{bash, exec = FALSE}
cd /Users/mcgaugheyd/data/scEiaD_modeling/hs111_mature_backeye
...
## scib scoring
```{r}
scib_files <- list.files(path = '~/data/scEiaD_modeling/hs111_mature_backeye/', full.names = TRUE) %>% grep('scib',.,value = TRUE)
...
## Assess Output
```{r}
obs <- data.table::fread('~/data/scEiaD_modeling/hs111_mature_backeye/hs111_mature_backeye_20240830_1000hvg_200e_30l.obs.csv.gz' )[,-1] %>% relocate(barcode) %>% 
...
```{r diff}
# pull in diff
diff_testing <- data.table::fread("~/data/scEiaD_modeling/hs111_mature_backeye/hs111_mature_backeye_20240830_2000hvg_200e_30l.difftesting.leiden3.csv.gz") %>% mutate(ENSEMBL = gsub("\\.\\d+","",names))
...
library(tidyverse)
pb <- data.table::fread('~/data/scEiaD_modeling/hs111_mature_backeye/hs111_mature_backeye_20240830_1000hvg_200e_30l.pseudoBulk.leiden3.csv.gz')
hvg <- data.table::fread('~/data/scEiaD_modeling/hs111_mature_backeye/hvg1000.csv.gz')[-1,]
rnames <- pb$V1
...
obs_labels_e <- obs_labels %>% 
  mutate(MCT_edit = 
           case_when(leiden3 %in% c(83,61, 9) ~ 'doublet',
                     leiden3 %in% c(10,
                                    84,
                                    2,
                                    7,
